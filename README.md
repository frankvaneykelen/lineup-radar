...
### Archiving Past Festivals

Festivals can be marked as archived to hide them from the main navigation while keeping them accessible via the archive page. This helps declutter the menu for upcoming events.

**To archive a festival:**

1. Open the festival's `settings.json` file (e.g., `docs/pinkpop/2026/settings.json`)
2. Set `"archived": true`
3. Regenerate the general pages:

   ```powershell
   .\scripts\regenerate_general.ps1
   ```
...
- All existing pages (lineup, artist pages, timetable, about) remain accessible via direct links

**To unarchive a festival:**

1. Set `"archived": false` in the festival's `settings.json`
...
- `scripts/regenerate_general.ps1` - Regenerates all general pages (homepage, archive, charts, FAQ)
...
The main landing page at `docs/index.html` is automatically generated by `scripts/generate_homepage.py`. It includes:

- Welcome message and project description
- Overview of features (filtering, discovery, diversity tracking)
- Links to all available festival lineups (automatically discovered from docs/ directory)
- Upcoming vs archived festival sections
- Dark mode support
- Responsive design for mobile and desktop

The homepage is automatically regenerated when you run `.\scripts\regenerate_general.ps1` or `.\scripts\regenerate_all.ps1`.
